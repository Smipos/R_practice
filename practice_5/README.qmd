...
data <- read.csv("C:/Users/smipo/OneDrive/Рабочий стол/practice_5/mir.csv-01.csv")
...
``` R
unsafe_dots <- subset(data, Privacy == " OPN")
num <- nrow(unsafe_dots)
...
``` R
wpa3 <- subset(data, Privacy == " WPA3 WPA2", select = c(BSSID, ESSID, Privacy))
...
6.  Отсортировать точки доступа по интервалу времени, в течении которого
    они находились на связи, по убыванию

``` R
wpa <- subset(data, Privacy == " WPA3 WPA2")
sorted_dots <- wpa %>%
